...
[Демонстрация сайта](https://star-burg-example-depl.work.gd/)
...
Для деплоя есть `docker-compose.yaml`, который собирает сайт и базу данных.

Для запуска нужно перейти в папку `docker` и выполнить
```
docker-compose build
docker-compose up
```

Возможно сделать запуск автоматическим, используя скрипты `systemd`

## Как запустить dev-версию сайта
В директории пользователя на сервере есть скрипт "deploy_star_burger.sh"

Для автоматического деплоя достаточно написать:
